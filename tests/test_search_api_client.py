...
class TestSearchApiClient(object):
    ...
    def test_create_index(self, search_client, rmock):
        rmock.put('http://baseurl/new-index', json={'status': 'ok'}, status_code=200)
    ...
    def test_delete_index(self, search_client, rmock):
        rmock.delete('http://baseurl/old-index', json={'status': 'ok'}, status_code=200)
    ...
    def test_set_alias(self, search_client, rmock):
        rmock.put('http://baseurl/new-alias', json={'status': 'ok'}, status_code=200)

        result = search_client.set_alias('new-alias', 'target')

        assert rmock.called
        assert result['status'] == 'ok'
        assert rmock.last_request.json() == {'type': 'alias', 'target': 'target'}
    ...
    def test_post_to_index_without_type_defaults_to_services(self, search_client, rmock):
        rmock.put('http://baseurl/g-cloud-9/services/12345', json={'message': 'acknowledged'}, status_code=200)
        result = search_client.index(
            'g-cloud-9',
            '12345',
            {'serialized': 'service'},
    ...
    def test_delete_to_delete_method_service_id(self, search_client, rmock, client_wait_for_response):
        rmock.delete(
            'http://baseurl/g-cloud/services/12345',
            json={'services': {'_id': '12345', '_index': 'g-cloud', '_type': 'services', '_version': 1, 'found': True}},
            status_code=200,
        )
    ...
    def test_delete_raises_if_http_error_not_404(self, search_client, rmock):
        rmock.delete(
            'http://baseurl/g-cloud/services/12345',
            json={'services': {'_id': '12345', '_index': 'g-cloud', '_type': 'services', '_version': 1, 'found': True}},
            status_code=400,
    ...
    def test_delete_returns_none_if_404(self, search_client, rmock):
        rmock.delete('http://baseurl/g-cloud/services/12345', status_code=404)

        assert search_client.delete(index='g-cloud', service_id='12345') is None

    def test_should_not_call_search_api_if_es_disabled(self, search_client, rmock):
        search_client._enabled = False
        rmock.put('http://baseurl/g-cloud/services/12345', json={'message': 'acknowledged'}, status_code=200)
        result = search_client.index(
            index_name='g-cloud',
            object_id='12345',
    ...
    def test_should_raise_error_on_failure(self, search_client, rmock):
        with pytest.raises(APIError):
            rmock.put('http://baseurl/g-cloud/services/12345', json={'error': 'some error'}, status_code=400)
            search_client.index(
                index_name='g-cloud',
                object_id='12345',
                serialized_object={'serialized': 'service'},
    ...
    @pytest.mark.parametrize(
        'index, doc_type', (('briefs-digital-outcomes-and-speciaists', 'briefs'), ('g-cloud-9', 'services'))
    )
    def test_search(self, search_client, rmock, index, doc_type):
        expected_response = {'documents': 'myresponse'}
        rmock.get(
            'http://baseurl/{}/{}/search?q=foo&filter_something=a&filter_something=b'.format(index, doc_type),
            json=expected_response,
            status_code=200,
        )
        result = search_client.search(index=index, doc_type=doc_type, q='foo', something=['a', 'b'])
        assert result == expected_response

    @pytest.mark.parametrize(
        'index, doc_type', (('briefs-digital-outcomes-and-specialists', 'briefs'), ('g-cloud', 'services'))
    )
    ...
    def test_search_with_blank_query(self, search_client, rmock, index, doc_type):
        rmock.get(
            'http://baseurl/{}/{}/search'.format(index, doc_type), json={'documents': 'myresponse'}, status_code=200
        )
    ...
    def test_get_frontend_params_from_search_api_url(
        self, search_client, rmock, search_api_url, expected_frontend_params
    ):
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            (
                'http://localhost/briefs-digital-outcomes-and-specialists-2/briefs/search',
                'briefs-digital-outcomes-and-specialists-2',
            ),  # NOQA
            ...
            (
                'https://search-api.preview.marketplace.team/briefs-digital-outcomes-and-specialists-2/briefs/search',
                'briefs-digital-outcomes-and-specialists-2',
            ),  # NOQA
    ...
    def test_search_service_from_url(self, search_client, rmock):
        ...
        result = search_client.search_services_from_url(
            search_api_url='https://baseurl/g-cloud/services/search?lot=cloud-hosting', page=1
        )
    ...
    def test_search_service_from_url_id_only(self, search_client, rmock):
        ...
        result = search_client.search_services_from_url(
            search_api_url='https://baseurl/g-cloud/services/search?lot=cloud-hosting', id_only=True, page=1
        )
        assert result == {'services': 'myresponse'}
